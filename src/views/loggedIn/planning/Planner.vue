<template>
                  ...
                  >
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody class="divide-y divide-gray-200">
                        <tr v-for="event in filteredEvent" :key="event.id">
                          <td
  ...
  <section class="mt-20 mb-20 divide-y divide-gray-200"></section>
</template>
    ...
    const getEvents = async () => {
      await plannerService.getEvents(route.params.id).then((res) => {
        for (let i = 0; i < res.data.data.length; i++) {
          ...
          let startDate = res.data.data[i].start
            .split("T")
            .slice(0, 1)
            .join("");
          let endDate = res.data.data[i].start.split("T").slice(0, 1).join("");

          // recup les données de la response stockée dans planning pour mettre dans allEvents qui sera render
          filteredEvent.value.push({
            planning_id: res.data.data[i].id,
            travel_id: res.data.data[i].travel_id,
            start: startDate,
            end: endDate,
            hourStart: res.data.data[i].hourStart,
            hourEnd: res.data.data[i].hourEnd,
